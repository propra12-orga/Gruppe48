...
public class boardPanel extends JPanel{
	...
	public boardPanel(Field field )
	{
		...
		boardField = field;
		 try{
		     imgExit = ImageIO.read(new File ("exit.png"));
		     imgWall = ImageIO.read(new File ("wall.png"));
		     imgFree = ImageIO.read(new File ("free.png"));
		     imgBomb = ImageIO.read(new File ("bomb.png"));
		     imgPlayer = ImageIO.read(new File ("player.png"));
		     imgBoom = ImageIO.read(new File ("boom.png"));
	      }catch(IOException e)
		        {
	    ...
	    public void paintBoom(Graphics g)
	    {
	    	if(iBoomList.size() > 0)
	    	{
	    		for(int i = 0; i < iBoomList.size(); i++)
	    		{
	    			...
	    			{
	    				g.drawImage(imgBoom, ((int[])iBoomList.get(i).get(j))[0] * 32, ((int[])iBoomList.get(i).get(j))[1] * 32, 32, 32, null);
	    				//System.out.println(((int[])iBoomList.get(i).get(j))[0]);
	    			}
	    		}
	    	}
	    }
